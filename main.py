...
dl_dance = DataLoader()
dance_data_X_train, dance_data_y_train=dl_dance.next_batch(batch=False)
dance_data_X_val, dance_data_y_val=dl_dance.validation_data(batch=False)

#X_train = np.transpose(data_dict['X_train'],[0,2,1])
X_train = np.transpose(dance_data_X_train,[0,2,1])
y_train = data_dict['y_train'][:dl_dance.get_train_length()]
X_val = np.transpose(dance_data_X_val,[0,2,1])
y_val = data_dict['y_val'][:dl_dance.get_val_length()]
  ...
  if MDN:
    ...
    if export_to_csv:
      ...
      for count in range(sl):
          timestamp = seq_samp[60,count]
          for joint_id in range(20):
            x = seq_samp[joint_id, count]
            y = seq_samp[joint_id + 1, count]
            z = seq_samp[joint_id + 2, count]
            csvwriter.writerow([count, timestamp, joint_id, x, y, z])
  ...
  if export_shiny:
    ...
    for pre in np.arange(5,10):
      for b in np.arange(0,4,0.5):
        ...
        plt.close('all')
    np.savetxt(direc_pred+'dance_shiny.csv',seq_exp,delimiter=',')
    export_lab = np.stack(export_lab,axis=1).T    #
    np.savetxt(direc_pred+'dance_shiny_log.csv',export_lab,delimiter=',')
